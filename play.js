...
function show_card_list(id, card_lists) {
    show_dialog(id, (body) => {
        ...
        let append_header = (text) => {
            ...
            header.textContent = (typeof zh_t === "function" ? zh_t(text) : text)
...
function update_rollback_dialog() {
    ...
    rollback_header.textContent = "此回滚将撤销："
...
function show_score_summary() {
    show_dialog("score", (body) => {
        ...
        let append_score = (label, value) => {
            ...
            p.innerHTML = `${typeof zh_t === "function" ? zh_t(label) : label}: ${value > 0 ? "+" : ""}${value}`
            dl.appendChild(p)
        }
        ...
        let append_value = (label, value) => {
            let p = document.createElement("dd")
            p.className = "score_row"
            p.innerHTML = `${typeof zh_t === "function" ? zh_t(label) : label}: ${value}`
            dl.appendChild(p)
        }
        ...
        let append_string = (label) => {
            let p = document.createElement("dd")
            p.className = "score_row"
            p.innerHTML = `${typeof zh_t === "function" ? zh_t(label) : label}`
            dl.appendChild(p)
        }
